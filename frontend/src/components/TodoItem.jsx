  ...
  return (
    <div className="flex items-center gap-3 p-4 bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 group">
      ...
      {isEditing ? (
        ...
        <p
          className={`flex-1 text-gray-800 select-none ${isDone ? "line-through" : ""}`}
        >
          {todo}
        </p>
      )}
      <button
        className="opacity-0 group-hover:opacity-100 px-3 py-1 text-blue-600 hover:bg-blue-50 rounded-md transition-all duration-200 text-sm font-medium"
        {...(isEditing ? { onClick: saveEdit } : { onClick: startEdit })}
      >
